...
namespace API.Controllers
{
    ...
    public class CutiController : ControllerBase
    {
        ...
        {
            var dataGet = cutiRepository.Get();
            if (dataGet.Count == 0)
            {
        ...
        {
            var dataGet = cutiRepository.Get(id);
            if (dataGet == null)
            {
                return Ok(new { message = "Sukses mengambil data", statusCode = 200, data = "null" });
            }
        ...
        {
            var resultPut = cutiRepository.Put(cuti);
            if (resultPut > 0)
            {
        ...
        {
            var resultPost = cutiRepository.Post(cuti);
            if (resultPost > 0)
            {
                return Ok(new { message = "Sukses memperbaharui data", statusCode = 200 });
            }
        ...
        {
            var resultDelete = cutiRepository.Delete(id);
            if (resultDelete > 0)
            {
